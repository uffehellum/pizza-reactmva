...
describe('SendTelemetry', () => {
    ...
    it('bigger payload', (done) => {
        try {
            const request: TelemetryEvent = {
                event: 'my big event',
                payload: { contentlist: [
                    {id:'1235', shown:'success'},
                    {id:'456', shown:'success'},
                    {id:'789', shown:'success'},
                ]},
                session: 'my longsession',
                timestamp: new Date(),
            }
            const result = SendTelemetry(request, config)
            result.then((json: any) => {
...
describe('prod endpoint v1', () => {
    const url = 'https://vortex-sandbox.data.microsoft.com/collect/v1'

    it('reject without name attribute', (done) => {
        try {
            const payload = { x: 'abc' }
            ...
            fetch(url, params)
                .then(response => {
                    response.json()
                        .then(result => {
                            expect(result.rej).toEqual(1)
                            expect(result.acc).toEqual(0)
                            done()
                        })
                        .catch(e => done(e))
                })
    ...
    it('accept with name attribute', (done) => {
        try {
            ...
            fetch(url, params)
                .then(response => {
                    response.json()
                        .then(result => {
                            expect(result.rej).toEqual(0)
                            expect(result.acc).toEqual(1)
                            done()
                        })
                        .catch(e => done(e))
                })
...
pass:
$ curl https://vortex-sandbox.data.microsoft.com/collect/v1\
  -X POST -H "Content-Type: text/plain;charset=UTF-8" -d '{"name":"abc"}'pass:

pass:
$ curl https://vortex-sandbox.data.microsoft.com/collect/v1\
  -X POST -H "Content-Type: application/json" -d '{"name":"abc"}'
